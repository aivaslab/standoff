...
def make_env(envClass, player_config, configName=None, memory=1, threads=1, reduce_color=False, size=64,
    ...
    info_keywords = ['shouldAvoidBig', 'shouldAvoidSmall', 'correctSelection', 'selection', 'selectedBig', 'selectedSmall', 'selectedNeither',
    ...
    if vecMonitor:
        if path != "":
            ...
            env = VecMonitor(env, filename=configName + "-timesteps-"+str(rank), info_keywords=info_keywords)
    return env
