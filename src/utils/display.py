...
def make_pic_video(model, env, name, random_policy=False, video_length=50, savePath='', vidName='video.mp4', following="player_0", image_size=128):
    ...
    for i in range(video_length):
        images.append(img)
        print(env.action_spaces[following])
        action, _ = {agent: env.action_spaces[following].sample() for agent in env.agents} if random_policy else model.predict(obs)
